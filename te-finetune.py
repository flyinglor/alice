...
def train_FT(args, f=1):
    ...
    if not args.scratch:
        ...
        # Load state_dict
        student.load_state_dict(checkpoint['teacher'])
...
if __name__ == '__main__':
    ...
    if not args.disable_wandb:
        ...
        run = wandb.init(project=f"alice_ft_{ds}", 
                        name=f"te_{pretrain_type}_{args.pretrain_ds}_{ft}_bs{args.batch_size}_ep{args.epochs}_lr{args.lr}_{args.roi_x}^3", 
                        id=wandb_id,
                        resume='allow',
                        # dir=args.output_dir
